...
int channel_send_rma_spsc_buf(MPI_Channel *ch, void *data)
{
    ...
    do {
        ...
        MPI_Get_accumulate(NULL, 0, MPI_BYTE, &index_r, sizeof(int), MPI_BYTE, ch->my_rank, 0, 
        sizeof(int), MPI_BYTE, MPI_NO_OP, ch->win);
       MPI_Get_accumulate(NULL, 0, MPI_BYTE, &index_w, sizeof(int), MPI_BYTE, ch->my_rank, sizeof(int), 
        sizeof(int), MPI_BYTE, MPI_NO_OP, ch->win);
