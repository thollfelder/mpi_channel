...
/*
 * Implementation with nonblocking version of m&s queue
 * - Has best runtime and scales a lot better than implementation with MCS-Lock or MPI_EXCLUSIVE_LOCK
 */
...
/*
 * Test Version 1: Implementation with MPI_SHARED_LOCK and MCS-Lock 
 * - Better than version with exclusive lock but scales a lot worse than m&s queue
*/
/*
MPI_Channel *channel_alloc_rma_mpsc_buf(MPI_Channel *ch)
{
...
int channel_peek_rma_mpsc_buf(MPI_Channel *ch)
{
    ...
    // Get indices from receiver process
    if (MPI_Get_accumulate(NULL, 0, MPI_BYTE, index, 2, MPI_INT, ch->receiver_ranks[0], 0, 2*sizeof(int), MPI_BYTE, MPI_NO_OP, ch->win) != MPI_SUCCESS)
    {
        ERROR("Error in MPI_Get_accumulate()\n");
        return -1;    
    }
...
/*
Test Version 2: Implementation with MPI_EXCLUSIVE_LOCK
- A massive problem is that there is no fairness between the sender processes
- This can lead to delays of completion between senders of up to 20%
- Another problem is that runtime tests show worst results
*/
/*
MPI_Channel *channel_alloc_rma_mpsc_buf(MPI_Channel *ch)
{
