 ...
 * over its local variable. Since the Fetch-and
 * Op Operation is atomic exactly one sender fetches the current rank stored in LATEST_SENDER. As soon as a sender has the lock it can put the data to the receiver. Then
 * it updates the CURRENT_SENDER at the receiver to signal the receiver the origin of the data. To synchronize both sender and receiver the sender now spins over its local 
 * variable until the receiver has copied the data from the window to its passed data buffer and updates the spin variable of the origin sender. This step also guarantees
 * that the next lockholder must wait until the receiver has fetched the data from the previous lockholder. After the synchronization the sender process only needs to
 * check if another process has registered for the lock at the NEXT_SENDER. If its the case the calling sender can just wake the next sender up and return. If there is no
 * other sender registered at the NEXT_RANK it needs to CAS the LATEST_SENDER with its own rank.  If it matches, the sender replaces LATEST_SENDER with -1 meaning that
 * no other sender has registered for the lock and the calling sender can return. However if there is a number unlike the rank of the calling sender another sender will or
 * has registered for the lock. This means the calling sender must wait until the local NEXT_SENDER will be updated. After this happends the calling sender can wake the newly
 * registered sender up and return.
 * 
 ...
 * 
 * Why using passive target communication over MPI_Fence or MPI_{Post|Start|Complete|Wait}?
 ...
 *  over all senders is not possible 
...
int channel_send_rma_mpsc_sync(MPI_Channel *ch, void *data)
{
    ...
    MPI_Fetch_and_op(&ch->my_rank, &latest_sender, MPI_INT, ch->receiver_ranks[0], sizeof(int), MPI_REPLACE, ch->win);
    ...
    if (latest_sender != -1)
    {
        ...
        MPI_Accumulate(&ch->my_rank, 1, MPI_INT, latest_sender, displ_next_sender, sizeof(int), MPI_BYTE, MPI_REPLACE, ch->win);

        // Spin over local variable until woken up by previous sender holding the lock
        do
        {
            MPI_Win_sync(ch->win); // This ensures a memory update if using a non-unified memory model
        } while (lmem[SPIN_1] == -1);
    }
    ...
    MPI_Put(data, ch->data_size, MPI_BYTE, ch->receiver_ranks[0], displ_data, ch->data_size, MPI_BYTE, ch->win);
    ...
    if (lmem[NEXT_SENDER] == -1)
    {
        ...
        MPI_Compare_and_swap(&minus_one, &ch->my_rank, &latest_sender, MPI_INT, ch->receiver_ranks[0], sizeof(int), ch->win);
    ...
    MPI_Get_accumulate(NULL, 0, MPI_BYTE, &next_sender, 1,MPI_INT, ch->my_rank, displ_next_sender, sizeof(int), MPI_BYTE, MPI_NO_OP, ch->win);

    // Notify next sender by updating first spinning variable with a number unlike -1
    MPI_Accumulate(&ch->my_rank, 1, MPI_INT, next_sender, 0, 1, MPI_INT, MPI_REPLACE, ch->win);
...
int channel_receive_rma_mpsc_sync(MPI_Channel *ch, void *data)
{
    ...
    MPI_Fetch_and_op(&minus_one, &current_sender, MPI_INT, ch->receiver_ranks[0], 0, MPI_REPLACE, ch->win);

    // Wake up current sender by updating second spinning variable with a number unlike -1
    MPI_Accumulate(&ch->my_rank, 1, MPI_INT, current_sender, sizeof(int), sizeof(int), MPI_BYTE, MPI_REPLACE, ch->win);
