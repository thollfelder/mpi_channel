...
int main(int argc, char *argv[])
{
    ...
    //test_case(PT2PT_SPSC_UNSYNCHRONIZED_BUFFERED_SM);

    /*
    int x = 5;
    MPI_Channel* ch = channel_alloc(sizeof(int), 5, PT2PT_SPSC_UNSYNCHRONIZED_BUFFERED, MPI_COMM_WORLD, 0);
    channel_send_multiple(ch, &x, 2);
    channel_receive_multiple(ch, &x, 1);

    sleep(1);
    if (ch->my_rank == 1) {
        printf("Can be send: %i \n", channel_check(ch));
    }
    if (ch->my_rank == 0) {
        printf("Can be received: %i \n", channel_peek(ch));
    }
   ...
   int x = 0;
    ...
    MPI_Channel* ch =channel_alloc(sizeof(int), 0, PT2PT_SPSC_SYNCHRONIZED, MPI_COMM_WORLD, 0);

    if (ch->my_rank == 1) {
        x = 1;
        channel_send(ch, &x);
        printf("Sending through channel\n");
    }
    ...
    if (ch->my_rank == 0) {
        int ret = channel_peek(ch, &x);
        printf("Peeking channel, Return: %i, data: %i\n", ret, x);
        x = 0;
    }
    ...
    if (ch->my_rank == 0) {
        printf("Receiving from channel, Return: %i, data: %i\n", channel_receive(ch, &x), x);
    }
